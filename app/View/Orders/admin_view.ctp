...
//cloudkassir
if ($data['PaymentMethod']['cloudkassir'] == 1) 
{   
    echo '<form method="post" action="/vamshop/payment/cloudkassir/manual_send_receipt/">
    <input type="hidden" name="order_id" value="'.$data['Order']['id'].'">';

    if ($data['Order']['cloudkassir'] == 1)
    {
        echo '<input type="hidden" name="Type" value="IncomeReturn">
        <input type="submit" value="Отправка чека возврата прихода" class="btn btn-default">
        </form>';
    }
    elseif ($data['Order']['cloudkassir'] == 0)
    {
        echo '<input type="hidden" name="Type" value="Income">
        <input type="submit" value="Отправка чека прихода" class="btn btn-default">
        </form>';
    };
};
//cloudkassir
